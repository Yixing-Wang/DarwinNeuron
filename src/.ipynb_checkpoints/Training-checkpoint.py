...
from src.LandscapeAnalysis.LossSurfacePlotter import log_loss_grids, log_loss_plot
import inspect
...
def train_loop_snn(my_model, train_dataloader, val_dataloader, loss_fn, device, run, epoch, results_path=None, loss_plotter=None):
    """ one epoch of training, going through all the batches once
    """    
    for batch, (x, y) in enumerate(train_dataloader):
        x, y = x.to(device), y.to(device)

        update_args = inspect.signature(my_model.update).parameters
        if len(update_args) >= 3:
            my_model.update(x, y, loss_fn)
        else:
            # Default: Evolutionary Algorithm update
        ...
        best_model = my_model.get_best_model()
        ...
        val_stats = evaluate_snn(my_model.get_best_model(), val_dataloader, loss_fn, device)
    ...
    if loss_plotter is not None:
        ...
        center = parameters_to_vector(my_model.get_best_model().parameters()).cpu().numpy()
        f = get_parameter_to_loss_fn(val_dataloader, my_model.get_best_model(), loss_fn, device)
