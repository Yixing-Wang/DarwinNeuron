...
from ..Utilities import SNNStats
...
class PPSOModel(EA):
    ...
    def _pso_step(self, losses: torch.Tensor) -> None:
        ...
        if best_loss > self._loss_of(self.mean):
            return
        ...
        max_norm = 0.1
        if self.velocity.norm() > max_norm:
            self.velocity = self.velocity / self.velocity.norm() * max_norm
